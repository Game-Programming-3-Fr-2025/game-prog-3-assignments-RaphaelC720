...
public class GhostBirdyScript : MonoBehaviour
{
    ...
    private const string PREV_KEY_BEST = "LastGhostRun";

    [SerializeField] private PlayerScript3 player;
    private void Awake()
    {
    ...
    private void OnFinishLineCrossed(bool runStarting)
    {
        if (runStarting)
        {
            ...
            _replay.PlayRecording(RecordingType.Last, Instantiate(_ghostPrefab));
        }
        else
        {
            _replay.FinishRun();
    ...
    private void EndRun()
    {
        ...
        var isLast = _replay.FinishRun(true);
        _replay.StopReplay();
        ...
        if (isLast)
        {
            LastAttemptText.SetActive(true);
            if (_replay.GetRun(RecordingType.Last, out var lastRun))
            {
                try
                {
                    var serialized = lastRun.Serialize();
                ...
                if (_lastTimeText != null) 
                    _lastTimeText.text = FormatTime(_elapsedTime);
        ...
        if (_timerText != null) _timerText.text = FormatTime(_elapsedTime);
        Debug.Log($"Run ended. Time: {_elapsedTime:F3}s. LastRun: {isLast}");
    }
